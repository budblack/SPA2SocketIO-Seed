					...
					getInline: function (item, template) {
						...
						if (item.src) {
							...
							if (el.length) {
								...
								// If target element has parent - we replace it with placeholder and put it back after
								// popup is closed
						    ...
						    onLoadComplete = function () {
							    if (item) {
								    if (item.img[0].complete) {
									    ...
									    // if image complete check fails 200 times (20 sec), we assume that there was
									    // an error.
									    guard++;
									    if (guard < 200) {
						...
						_mfpOn(
							'BuildControls' + ns, function () {
								if (mfp._allowZoom()) {
									...
									// Basically, all code below does is clones existing image, puts in on top of the
									// current one and animated it
 ...
 * });
		 ...
		 *
		 *
		 * Note that it's a very basic and simple implementation, it blocks ghost click on the same element where it
		 * was bound. If you need something more advanced, use plugin by FT Labs https://github.com/ftlabs/fastclick
		 *
		 */
		...
		(
			function () {
				...
				// As Zepto.js doesn't have an easy way to add custom events (like jQuery), so we implement it in this
				// way
